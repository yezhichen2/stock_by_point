...
class BuySelector(object):
    ...
    @classmethod
    def search(cls, select11, select12,  select21, select31):
        ...
        for index, row in input_df.iterrows():
            ...
            match12 = BuySelector.match(target_index=target_dt30, target_df=select12, selected_col_name="s005")
            match21 = BuySelector.match(target_index=target_dt60, target_df=select21, selected_col_name="s004")
            match31 = BuySelector.match(target_index=target_dtd, target_df=select31, selected_col_name="s006")

            if match12 and match21 and match31:
                yield index
